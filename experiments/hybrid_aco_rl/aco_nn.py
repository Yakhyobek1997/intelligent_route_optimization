...
def solve():
    global n, trails, trails2, c, bestTourLength, bestTour
...
def data_init():
    global n, m, trails, trails2, ants, c, X, W, W2, sess, train, cost
    ...
    W = tf.Variable(tf.random_normal([n, n]), name='weight')
    W2 = tf.Variable(tf.random_normal([n, 1]), name='weight')

    # Hypothesis
    layer = tf.matmul(X, W)
    hypothesis = tf.nn.relu(tf.matmul(layer, W2))
...
def prob_to(ant):
    ...
    for i in range(n):
        if not ant.visited[i]:
            total += (trails[town][i] + trails[town][i]) * pow(1.0 / get_distance(town, i), beta)
    ...
    for i in range(n):
        if ant.visited[i]:
            ...
            prob = (trails[town][i] + trails[town][i]) * pow(1.0 / get_distance(town, i), beta)
            try:
...
def update_trails():
    ...
    for ant in ants:
        ...
        W.assign(w_data)
        w_data2 = [[trails2[ant.tour[i - 1]][ant.tour[i]]] for i in range(n)]
        W2.assign(w_data2)

        cost_val, w_val, w_val2, _ = sess.run([cost, W, W2, train], feed_dict={X: x_data})
        ...
        for i in range(n):
            if not math.isnan(float(w_val2[i][0])):
                trails2[ant.tour[i - 1]][ant.tour[i]] = float(w_val2[i][0])
