"""Ant Colony Optimization solver for coordinate-based TSP datasets."""
...
DEFAULT_DATASET = PROJECT_ROOT / "data" / "cities_30.txt"
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "aco_result.png"
...
class Solution:
    """Best route and metrics returned by the solver."""
...
class LinearPheromoneACO:
    ...
    def tour_length(self, tour: Sequence[int]) -> float:
        """Return the length of a circular route."""
        return sum(
    ...
    def solve(self, iterations: int = 200) -> Solution:
        """Run the optimizer and return its best solution."""
        if iterations < 1:
...
def save_result_plot(
    ...
    """Save the best route and convergence history as one image."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Intelligent Route Optimization: solve a coordinate-based TSP "
